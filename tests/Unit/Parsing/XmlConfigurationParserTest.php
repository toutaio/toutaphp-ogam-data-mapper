...
final class XmlConfigurationParserTest extends TestCase
{
    ...
    public function testParsePropertiesWithInlineValues(): void
    {
        $xml = <<<XML
            <?xml version="1.0" encoding="UTF-8"?>
            <configuration>
        ...
        $this->assertSame('mysql', $config->getProperty('db.driver'));
        $this->assertSame('localhost', $config->getProperty('db.host'));
        $this->assertSame('3306', $config->getProperty('db.port'));
    }

    public function testParsePropertiesFromResourceFile(): void
    {
        ...
        $propsPath = $this->tempDir . '/database.properties';
        file_put_contents($propsPath, $propertiesContent);
        ...
        $this->assertSame('sqlite', $config->getProperty('db.driver'));
        $this->assertSame(':memory:', $config->getProperty('db.database'));
        $this->assertSame('TestApp', $config->getProperty('app.name'));
    }

    public function testParsePropertiesFromResourceFileWithAbsolutePath(): void
    {
        $propertiesContent = <<<PROPS
            db.host=prodhost
            db.port=5432
        ...
        $propsPath = $this->tempDir . '/config.properties';
        file_put_contents($propsPath, $propertiesContent);

        $xml = <<<XML
            <?xml version="1.0" encoding="UTF-8"?>
            <configuration>
                <properties resource="{$propsPath}"/>
            </configuration>
            XML;

        $config = $this->parser->parseXml($xml);

        $this->assertSame('prodhost', $config->getProperty('db.host'));
        $this->assertSame('5432', $config->getProperty('db.port'));
    }

    public function testParsePropertiesWithResourceAndInlineOverrides(): void
    {
        // Resource file has base values
        $propertiesContent = <<<PROPS
            db.driver=mysql
            db.host=localhost
            db.port=3306
        ...
        $propsPath = $this->tempDir . '/base.properties';
        file_put_contents($propsPath, $propertiesContent);

        // Inline properties override resource values
        $xml = <<<XML
            <?xml version="1.0" encoding="UTF-8"?>
            <configuration>
                <properties resource="base.properties">
                    <property name="db.host" value="production-server"/>
                    <property name="db.port" value="3307"/>
                </properties>
            </configuration>
            XML;
        ...
        $this->assertSame('mysql', $config->getProperty('db.driver'));
        $this->assertSame('production-server', $config->getProperty('db.host'));
        $this->assertSame('3307', $config->getProperty('db.port'));
    }

    public function testPropertyPlaceholderSubstitution(): void
    {
        $xml = <<<XML
            <?xml version="1.0" encoding="UTF-8"?>
            <configuration>
                <properties>
                    ...
                    <property name="db.host" value="localhost"/>
                    <property name="db.port" value="3306"/>
                    <property name="db.name" value="myapp"/>
                </properties>
                <environments>
                    <environment id="test">
                        <transactionManager type="JDBC"/>
    ...
    public function testPropertiesWithEmptyResourceFileFails(): void
    {
        $xml = <<<XML
            <?xml version="1.0" encoding="UTF-8"?>
            <configuration>
                <properties resource="nonexistent.properties"/>
            </configuration>
            XML;

        $this->expectException(RuntimeException::class);
        $this->expectExceptionMessage('Properties file not found');

        $this->parser->parseXml($xml, $this->tempDir);
    }

    public function testPropertiesFileWithCommentsAndEmptyLines(): void
    {
        $propertiesContent = <<<PROPS
            # This is a comment
            db.driver=mysql
        ...
        $propsPath = $this->tempDir . '/commented.properties';
        file_put_contents($propsPath, $propertiesContent);

        $xml = <<<XML
            <?xml version="1.0" encoding="UTF-8"?>
            <configuration>
                <properties resource="commented.properties"/>
            </configuration>
            XML;
    ...
    public function testGetPropertyReturnsNullForUnknownProperty(): void
    {
        $xml = <<<XML
            ...
            <configuration>
                <properties>
                    <property name="known" value="value"/>
                </properties>
            </configuration>
            XML;
        ...
        $this->assertNull($config->getProperty('unknown'));
    }

    public function testGetPropertyWithDefaultValue(): void
    {
        $xml = <<<XML
            <?xml version="1.0" encoding="UTF-8"?>
            <configuration>
        ...
        $this->assertSame('default', $config->getProperty('unknown', 'default'));
        $this->assertSame('value', $config->getProperty('known', 'default'));
    }
